<script>
  ...
  let context = '';
  let selectedRoundId = '';
  let isGlobal = projectId === null; // projectIdがnullの場合は自動的にグローバルノード
  let loading = false;
  let error = '';
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // projectIdがnullの場合は自動的にグローバルノードにする
      if (projectId === null || isGlobal) {
      ...
      error = ''; // 成功時にエラーメッセージをクリア
      title = '';
      context = '';
      selectedRoundId = '';
      isGlobal = projectId === null; // projectIdがnullの場合は自動的にグローバルノード
    } catch (err) {
...
<div class="node-create-form">
  ...
  <form on:submit|preventDefault={handleSubmit}>
    ...
    {:else}
      <div class="form-group">
        <p class="info">グローバルノードとして作成されます</p>
      </div>
    {/if}
